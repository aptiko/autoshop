...
class RepairTestCase(APITestCase):
    ...
    def test_repair_list(self):
        ...
        response = self.client.get('/users/{}/repairs/'.format(self.alice.id))
        expected = (
    ...
    def test_user_repair_list(self):
        ...
        logged_in = self.client.login(username='bob', password='topsecret')
        self.assertTrue(logged_in)
        response = self.client.get('/users/{}/repairs/'.format(self.alice.id))
        self.assertEqual(response.status_code, 403)

        # But David, an admin, should
        logged_in = self.client.login(username='david', password='topsecret')
        self.assertTrue(logged_in)
        response = self.client.get('/users/{}/repairs/'.format(self.alice.id))
        expected = (
            '[{{"id":{},"assigned_user":{},"date":"2017-04-27",'
    ...
    def test_repair_detail(self):
        logged_in = self.client.login(username='alice', password='topsecret')
        self.assertTrue(logged_in)
        response = self.client.get('/repairs/{}/'.format(self.repair1.id))
    ...
    def test_mark_repair_complete(self):
        logged_in = self.client.login(username='alice', password='topsecret')
        self.assertTrue(logged_in)
        response = self.client.put('/repairs/{}/'.format(self.repair1.id),
    ...
    def test_delete_repair(self):
        ...
        response = self.client.delete(
            '/repairs/{}/'.format(self.repair1.id))
    ...
    def test_permissions_anonymous(self):
        ...
        # Delete repair
        response = self.client.delete('/repairs/{}/'.format(self.repair1.id))
        self.assertEqual(response.status_code, 403)
...
class UserTestCase(APITestCase):
    ...
    def test_user_detail(self):
        logged_in = self.client.login(username='david', password='topsecret')
        self.assertTrue(logged_in)
        response = self.client.get('/users/{}/'.format(self.alice.id))
        expected = ('{{"id":{},"username":"alice","is_staff":false}}'
    ...
    def test_update_user(self):
        logged_in = self.client.login(username='david', password='topsecret')
        self.assertTrue(logged_in)
        response = self.client.put('/users/{}/'.format(self.alice.id),
                                   data={'username': 'aliki',
                                         'is_staff': False})
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_user(self):
        logged_in = self.client.login(username='david', password='topsecret')
        self.assertTrue(logged_in)
        response = self.client.delete('/users/{}/'.format(self.alice.id))
        self.assertEqual(response.status_code, 204)
    ...
    def test_permissions_other_regular_user(self):
        ...
        response = self.client.get('/users/{}/'.format(self.alice.id))
        self.assertEqual(response.status_code, 403)
        response = self.client.get('/users/{}/'.format(self.charlie.id))
        self.assertEqual(response.status_code, 403)
        ...
        response = self.client.put('/users/{}/'.format(self.alice.id),
                                   data={'username': 'aliki',
                                         'is_staff': False})
        self.assertEqual(response.status_code, 403)
        response = self.client.put('/users/{}/'.format(self.charlie.id),
        ...
        response = self.client.delete('/users/{}/'.format(self.alice.id))
        self.assertEqual(response.status_code, 403)
        response = self.client.delete('/users/{}/'.format(self.charlie.id))
        self.assertEqual(response.status_code, 403)
    ...
    def test_permissions_admin(self):
        ...
        response = self.client.delete('/users/{}/'.format(self.charlie.id))
        self.assertEqual(response.status_code, 204)
        response = self.client.delete('/users/{}/'.format(self.david.id))
        self.assertEqual(response.status_code, 204)
